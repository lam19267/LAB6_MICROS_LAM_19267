...
reset2 macro ; Se crea una macro para reiniciar el timer2
    BANKSEL PR2
...
isr:
    ...
    call int_t1 ; Se llama la subrutina del timer1
    btfsc ((INTCON) and 07Fh), 2 ; Se verifica el overflow del timer0
    call int_t0 ; Se llama la subrutina del timer0

    BANKSEL PIR1
    btfsc ((PIR1) and 07Fh), 1 ; Verificar el overflow del timer2
    call int_t2 ; Llamar la interrupcion del timer2
    BANKSEL TMR2
    bcf ((PIR1) and 07Fh), 1 ; Se limpia la bandera del timer2
...
loop:
    reset2
    BANKSEL PORTA
    call dividir_nibble ; Se llama a la division de los nibbles

    btfss pestaneo, 0
    call preparar_nibble ; Se mandan los nibbles a cada display

    btfsc pestaneo, 0
    call parpadeo ; Se llama al papaedeo
goto loop

;---------------subrutinas----------
dividir_nibble: ; Se separan los nibbles para la primera parte
    movf contador, w
    andlw 00001111B
    movwf nibble
    swapf contador, w ; Se cambian los ultimos 4 bits por los primeros
    andlw 00001111B
    movwf nibble+1
    return

preparar_nibble: ; Se mandan los valores que se quieren desplegar en el 7 segmentos
    movf nibble, w
    call sietes
